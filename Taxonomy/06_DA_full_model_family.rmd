...
```{r, message=F}
list.of.packages <- c("ggplot2", "dplyr", "reshape2", "emmeans", "lattice",
...
phyl_yy<-readRDS("Taxonomy/RDS/Phyloseq_filtered_family.rds")
```
...
Remove Post_Abx and keep only taxa with rel. ab. > 0 in at least 15% of samples.
...
*NO POST ABX*
```{r}
# Check tha the taxa name are == to name in df
all(tax_name == names(df_m)[1:21])
...
for(i in 1:length(tax_name)){
  # log 
  path<-"/Taxonomy/Figures/Residuals_family_full_model_noPostAbx/LOG/"
  dir.create("Taxonomy/Figures/Residuals_family_full_model_noPostAbx/LOG", recursive = T)
  ...
  dev.off()
  # LOG ZERO
  path<-"Taxonomy/Figures/Residuals_family_full_model_noPostAbx/LOGZERO/"
  dir.create("Taxonomy/Figures/Residuals_family_full_model_noPostAbx/LOGZERO", recursive = T)
  x<-paste0(path, tax_name[i], ".png", sep = "")
  trellis.device(device="png", file = x)
  ...
  dev.off()  
  # CLR
  dir.create("Taxonomy/Figures/Residuals_family_full_model_noPostAbx/CLR/", recursive = T)
  path<-"Taxonomy/Figures/Residuals_family_full_model_noPostAbx/CLR/"
  x<-paste0(path, tax_name[i], ".png", sep = "")
  trellis.device(device="png", file = x)
  ...
  dev.off()  
  # ArcSin
  dir.create("Taxonomy/Figures/Residuals_family_full_model_noPostAbx/ArcSin/", recursive = T)
  path<-"Taxonomy/Figures/Residuals_family_full_model_noPostAbx/ArcSin/"
  x<-paste0(path, tax_name[i], ".png", sep = "")
  trellis.device(device="png", file = x)
  ...
  dev.off()  
  # LOGIT
  dir.create("Taxonomy/Figures/Residuals_family_full_model_noPostAbx/LOGIT/", recursive = T)
  path<-"Taxonomy/Figures/Residuals_family_full_model_noPostAbx/LOGIT/"
  x<-paste0(path, tax_name[i], ".png", sep = "")
  trellis.device(device="png", file = x)
...
```{r}
res<-read.delim("Taxonomy/Tables/Residuals_family_noAbx.txt", header = T, stringsAsFactors = F)
# Check how many model types are good
unique(res$best_no_PostAbx)
res$genus<-gsub(" ","", res$genus)

# Now select only the one for the diff transf
res_clr<-res[res$best_no_PostAbx=="clr",]$genus
res_log_zero<-res[res$best_no_PostAbx=="LogZero",]$genus
res_arcsin<-res[res$best_no_PostAbx=="ArcSin",]$genus
res_logbefore<-res[res$best_no_PostAbx=="LogBefore",]$genus
...
Extract the significant taxa from the CLR estimates.
...
ggsave("Taxonomy/Figures/CLR_onlyfdr_DA_family.svg", device = "svg", height = 20, width = 20, units = "cm")
...
ggsave("Taxonomy/Figures/CLR_onlyfdr_DA_family_yo.svg", device = "svg", height = 20, width = 20, units = "cm")
...
df_contr_sign<-subset(df_contr_clr, padjust <= 0.05) # re-create object
...
write.table(df_sign_methods, "Taxonomy/Tables/DA_family_.txt")
write.table(df_contr, "Taxonomy/Tables/DA_all_Family_.txt")
...
### Save image
```{r}
save.image("/Taxonomy/RData/14_DA_lm_full_model_family.RData")
```
